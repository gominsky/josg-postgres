...
// ------- SOLO QR --------
async function insertarAsistenciaMadrid({ alumno_id, evento_id, tipo, ubicacion }) {
  ...
  await db.query(
    `INSERT INTO asistencias (alumno_id, evento_id, fecha, hora, tipo, ubicacion)
     VALUES ($1, $2,
             ...
             $3, $4)`,
    [alumno_id, evento_id, tipo, ubicacion || null]
  );
...
async function handleFirmarQR(req, res) {
  ...
  try {
    ...
    if (!ev.rows[0]) return res.status(400).json({ error: 'Evento no válido o inactivo' });

    const r = await insertarAsistenciaMadrid({ alumno_id, evento_id, tipo: 'qr', ubicacion });
    if (r.yaFirmado) return res.status(400).json({ error: 'Ya se ha firmado asistencia para este evento' });

    res.json({ success: true, mensaje: 'Asistencia QR registrada' });
  } catch (err) {
