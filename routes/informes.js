...
// ======================
// ATRIL · CLASIFICACIÓN ACTUAL (sin periodo, sin asistencia)
// ======================

// Helper: inferir instrumento (mapea Violín → Violín I/II si el alumno está en esos grupos)
async function inferInstrumentoActual(client, alumnoId) {
  // 1) instrumento “principal” del alumno
  const rIns = await client.query(`
    ...
    LIMIT 1
  `, [alumnoId]);
  let base = (rIns.rows[0]?.nombre || '').trim();

  // 2) si es violín, intenta sección por pertenencia a grupos; por defecto Violín II
  if (/^\s*viol[ií]n(\s|$)/i.test(base)) {
    const rSec = await client.query(`
      ...
      LIMIT 1
    `, [alumnoId]);
    base = rSec.rows[0]?.sec || 'Violín II';
  }

  return base || 'Varios';
}

// GET /informes/atril/actual?grupo_id=123
// Devuelve TODOS los alumnos activos del grupo, con instrumento (resuelto) y puesto si existe.
// Orden: Instrumento → Alumno.
router.get('/atril/actual', async (req, res) => {
  ...
  try {
    // Alumnos activos del grupo (mismo estilo que ya usas en informes) :contentReference[oaicite:2]{index=2}
    ...
    // Enriquecer con instrumento + puesto guardado (si lo hay)
    const out = [];
    for (const r of base) {
      const instrumento = await inferInstrumentoActual(client, r.alumno_id);
      const { rows: rP } = await client.query(`
        SELECT puesto
        FROM atril_clasificacion_actual
        WHERE grupo_id = $1 AND alumno_id = $2 AND instrumento = $3
        LIMIT 1
      `, [grupoId, r.alumno_id, instrumento]);
      out.push({
        alumno_id: r.alumno_id,
        nombre: r.nombre,
        instrumento,
        puesto: rP[0]?.puesto ?? null
      });
    }

    // Orden final: Instrumento → Alumno (alfabético)
    out.sort((a,b) =>
    ...
    console.error('[informes] GET /atril/actual error:', e);
    res.status(500).json({ error: 'Error cargando datos' });
  } finally {
    client.release();
  }
});
// PUT /informes/atril/actual
router.put('/atril/actual', express.json(), async (req, res) => {
  const { grupo_id, instrumento, items } = req.body || {};
  const grupoId = Number(grupo_id);
  if (!Number.isInteger(grupoId) || !instrumento || !Array.isArray(items)) {
    return res.status(400).json({ ok:false, error: 'grupo_id, instrumento e items requeridos' });
  }
  ...
  try {
    ...
    await client.query(
      `DELETE FROM atril_clasificacion_actual WHERE grupo_id=$1 AND instrumento=$2`,
      [grupoId, instrumento]
    );

    for (const it of items) {
      const alumnoId = Number(it.alumno_id);
      const puesto   = Number(it.puesto);
      if (!Number.isInteger(alumnoId) || !Number.isInteger(puesto) || puesto < 1) continue;

      await client.query(
        `INSERT INTO atril_clasificacion_actual (grupo_id, instrumento, alumno_id, puesto)
         VALUES ($1,$2,$3,$4)`,
        [grupoId, instrumento, alumnoId, puesto]
      );
    }
    ...
    console.error('[informes] PUT /atril/actual error:', e);
...
// Editor de clasificación actual (solo sirve la vista, los datos se piden por AJAX)
router.get('/atril/actual/editor', isAuthenticated, async (req, res) => {
