...
// REGISTRAR pago (aplica a UNA factura) + recálculo de estado
router.post('/facturas/:id/pagos', async (req, res) => {
  const facturaId = +req.params.id;
  const { importe, fecha, metodo = '', referencia = '', notas = '' } = req.body;

  if (!importe || !fecha) {
    return res.status(400).send('Faltan importe o fecha');
  }

  try {
    // Tomamos el proveedor de la factura
    const { rows: fac } = await db.query(
      `SELECT proveedor_id FROM facturas_prov WHERE id=$1`, [facturaId]
    );
    const proveedorId = fac[0]?.proveedor_id || null;
    ...
    const { rows: pago } = await db.query(
      `INSERT INTO pagos_prov (proveedor_id, fecha, importe_total, metodo, referencia, notas)
       VALUES ($1,$2,$3,$4,$5,$6)
       RETURNING id`,
      [proveedorId, fecha, Number(importe), metodo.trim(), referencia.trim(), notas.trim()]
    );

    // 2) Aplicar pago a la factura
    await db.query(
      `INSERT INTO pagos_prov_aplicaciones (pago_id, factura_id, importe_aplicado)
       VALUES ($1,$2,$3)`,
      [pago[0].id, facturaId, Number(importe)]
    );
    ...
    console.error(e);
    res.redirect(`/contabilidad/facturas/${facturaId}?error=pago`);
  }
});
...
/* ===================== Pagos: nuevo (distribuir en varias facturas) ===================== */
...
/* ===================== Pagos: guardar (con distribución) ===================== */
